...
def calculate_w(mi, sigma, pi):
    Wi = divide_matrix_by_x(inverse(sigma), -2)
    wi = multiply_matrices(inverse(sigma), mi)
    wi0 = math.log(pi) - (math.log(abs(modulus(sigma))) / 2) - divide_matrix_by_x(multiply_matrices(multiply_matrices(transpose(mi), inverse(sigma)), mi), 2)[0][0]
    return [Wi, wi, wi0]

def calculate_gix(param, x):
    Wi, wi, wi0 = tuple(param)
...
if __name__ == '__main__':
    ...
    print("Case 4: " + (" vs ".join(class_names)))
    ...
    for i in range(len(all_datasets)):
        params.append(calculate_w(all_means[i], all_covariance_matrices[i], all_probabilities[i]))
    ...
    plt.savefig(dirname + "Case4 " + (" vs ".join(class_names)) + ".png", format='png')
    
    plt.figure(2)
    plot_contour_graph(calculate_gix, params, margin)
    plot_dataset(class_names, all_training_sets)
    plt.legend(loc='upper right')
    plt.savefig(dirname + "Case4 " + (" vs ".join(class_names)) + " contour.png", format='png')
    print("Done.")
